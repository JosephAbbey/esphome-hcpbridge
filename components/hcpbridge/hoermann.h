...
// workaround as my Supramatic did not Report the Status 0x0A when it's en vent
// Position When the door is at position 0x08 and not moving Status get changed
// to Ventig.
...
private:
  HoermannCommand(uint16_t commandRegPlus2Value, uint16_t commandEndPlus2Value,
                  uint16_t commandRegPlus3Value,
                  uint16_t commandEndPlus3Value) {
...
public:
  ...
  void handleModbus();
  Modbus::ResultCode onRequest(Modbus::FunctionCode fc,
...
private:
  ...
  unsigned long commandWrittenOn =
      0; // When was last command written (wait 100ms before end of command is
         // transmitted)
};
#endif