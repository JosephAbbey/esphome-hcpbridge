// Credits to https://github.com/Gifford47/HCPBridgeMqtt for the initial code
// base
...
const HoermannCommand HoermannCommand::STARTOPENDOOR =
    HoermannCommand(0x0210, 0x0110, 0x0000, 0x0000); // Typo 0201
const HoermannCommand HoermannCommand::STARTCLOSEDOOR =
    HoermannCommand(0x0220, 0x0120, 0x0000, 0x0000);
const HoermannCommand HoermannCommand::STARTSTOPDOOR =
    HoermannCommand(0x0240, 0x0140, 0x0000, 0x0000);
const HoermannCommand HoermannCommand::STARTOPENDOORHALF =
    HoermannCommand(0x0200, 0x0100, 0x0400, 0x0400);
const HoermannCommand HoermannCommand::STARTVENTPOSITION =
    HoermannCommand(0x0200, 0x0100, 0x4000, 0x4000);
const HoermannCommand HoermannCommand::STARTTOGGLELAMP =
    HoermannCommand(0x0100, 0x0800, 0x0200, 0x0200);
const HoermannCommand HoermannCommand::WAITING =
    HoermannCommand(0x0000, 0x0000, 0x0000, 0x0000);

TaskHandle_t modBusTask;
void modbusServeTask(void *parameter);

void DelayHandler(void) { HoermannGarageEngine::getInstance().handleModbus(); }
...
void HoermannGarageEngine::setup(int8_t rx, int8_t tx, int8_t rts) {
  ...
  xTaskCreatePinnedToCore(modbusServeTask, /* Function to implement the task */
  ...
  mb.onRequest([this](Modbus::FunctionCode fc,
                      const Modbus::RequestData data) -> Modbus::ResultCode {
...
void HoermannGarageEngine::handleModbus() { mb.task(); }

Modbus::ResultCode
HoermannGarageEngine::onRequest(Modbus::FunctionCode fc,
                                const Modbus::RequestData data) {
  this->state->recordModbusResponse();

  // Command Requst (Internal State representation)
  if (fc == Modbus::FC_READWRITE_REGS && data.regWrite.address == 0x9C41 &&
      data.regWriteCount == 0x02 && data.regRead.address == 0x9CB9 &&
      data.regReadCount == 0x08) {
  ...
  else if (fc == Modbus::FC_READWRITE_REGS && data.regWrite.address == 0x9C41 &&
           data.regWriteCount == 0x02 && data.regRead.address == 0x9CB9 &&
           data.regReadCount == 0x02) {
  ...
  // BusScan
           ...
           data.regWriteCount == 0x03 && data.regRead.address == 0x9CB9 &&
           data.regReadCount == 0x05) {
...
void HoermannGarageEngine::setCommandValuesToRead() {
  ...
  if (nextCommand != nullptr) {
    ...
    } else if (commandWrittenOn != 0 &&
               (commandWrittenOn + SIMULATEKEYPRESSDELAYMS) < millis()) {
...
uint16_t HoermannGarageEngine::onDoorPositonChanged(TRegister *reg,
                                                    uint16_t val) {
  // on First Byte changed (current)
  if ((reg->value & 0x00FF) != (val & 0x00FF)) {
    this->state->setCurrentPosition((float)(val & 0x00FF) / 200.0f);
    if ((this->state->gotoPosition > 0.0f &&
         this->state->state == HoermannState::State::CLOSING &&
         this->state->gotoPosition >= this->state->currentPosition) ||
        (this->state->gotoPosition > 0.0f &&
...
uint16_t HoermannGarageEngine::onCurrentStateChanged(TRegister *reg,
                                                     uint16_t val) {
  // on First Byte changed
  if (((reg->value & 0xFF00) != (val & 0xFF00))) {
    ESP_LOGI(TAG_HCI,
             "onCurrentStateChanged. address=%x, value=%x (actual: %x)",
             reg->address.address, val, (val & 0xFF00) >> 8);
    ...
    case 0x00:
      if (this->state->currentPosition == this->state->targetPosition &&
          (int)(this->state->currentPosition * 200) == VENT_POS) {
        this->state->setState(HoermannState::State::VENT);
      } else {
...
uint16_t HoermannGarageEngine::onLampState(TRegister *reg, uint16_t val) {
  ...
  if ((reg->value & 0x00FF) != (val & 0x00FF)) {
    ESP_LOGI(TAG_HCI, "onLampState. address=%x, value=%x", reg->address.address,
             val);
 ...
 */
void HoermannGarageEngine::setCommand(bool cond,
                                      const HoermannCommand *command) {
  if (cond) {
    if (nextCommand != nullptr) {
...
void HoermannGarageEngine::toogleDoor() {
  ...
  setCommand(this->state->currentPosition >= 1,
             &HoermannCommand::STARTCLOSEDOOR);
}
...
void HoermannGarageEngine::turnLight(bool on) {
  setCommand((on && !this->state->lightOn) || (!on && this->state->lightOn),
             &HoermannCommand::STARTTOGGLELAMP);
}
...
void HoermannGarageEngine::setPosition(int setPosition) {
  // First and last movement segments seem a bit inconsistent on Promatic4, so
  // it's better to leave it to fully open or close.
  if (setPosition <= 5)
    closeDoor();
    ...
    setCommand(this->state->currentPosition < this->state->gotoPosition,
               &HoermannCommand::STARTOPENDOOR);
    setCommand(this->state->currentPosition > this->state->gotoPosition,
               &HoermannCommand::STARTCLOSEDOOR);
  }
}
...
void HoermannState::clearChanged() { this->changed = false; }
...
void HoermannState::setValid(bool isValid) { this->valid = isValid; }
